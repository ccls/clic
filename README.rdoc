...
* ruby <= 1.8.7 - primarily due to 1.9.1 differences
* rubygems
* ImageMagick (which may require {MacPorts}[http://www.macports.org/])
  * ImageMagick is used to image manipulation
* curl - for the installation of rvm (which isn't really a requirement)
...
* {packet}[http://packet.rubyforge.org] - requirement for BackgrounDRb (not using anymore)
...
* {gravatar}[http://rubygems.org/gems/gravatar] - for user icons (No longer used)
...
* {jakewendt-simply_photos}[http://github.com/jakewendt/simply_photos]
* {jakewendt-simply_documents}[http://github.com/jakewendt/simply_documents]
* {jakewendt-simply_helpful}[http://github.com/jakewendt/simply_helpful]
* {jakewendt-html_test}[http://github.com/jakewendt/html_test] - html testing and validation
* {jakewendt-rdoc_rails}[http://github.com/jakewendt/rdoc_rails] - simple plugin to add ActiveRecord associations to rdoc
* {jrails}[http://github.com/aaronchi/jrails] (pending) - jquery helper methods (trying to not have any inline javascript so may never really use jrails' methods, but may use the jrails.js.)


== Required and included plugins

* {backgroundrb}[http://github.com/gnufied/backgroundrb] - background job server and scheduler (includes minor mods to init.rb) (not used anymore)
...
then we continue ...
 ...
 #	We don't use backgroundrb at this point
...
An HTML validator like {Validator S.A.C.}[http://habilis.net/validator-sac/] should be installed locally for local HTML page validations.  This can be done over the internet, but can be slow and sometimes timeout. 

If Validator is not installed or Web Sharing is not enabled, testing will produce many ...

 Errno::ECONNREFUSED: Connection refused - connect(2)

I've made some mods in {html_test}[http://github.com/jakewendt/html_test] to check if a local version is running so this should never happen.
...
Also has a tendency to crash out silently.
 ...
 rake "sunspot:reindex[,Subject]" --trace



For now, use

 rake "sunspot:reindex[,Subject]"

instead of just 

 rake sunspot:reindex

as for whatever reason, some of the unrelated gem stuff isn't loaded.
