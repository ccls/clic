...
class ActionDispatch::CapybaraIntegrationTest < ActionDispatch::IntegrationTest
	...
	def block_unknown_urls
		page.driver.block_unknown_urls
		page.driver.allow_url("ajax.googleapis.com")
		page.driver.allow_url("tinymce.cachefly.net")
		#	page.driver.allow_url("s3.amazonaws.com")
	end
...
#	end
	...
	def login_as( user=nil )
		...
		if !username.blank?
			...
			page.visit new_user_session_path()	#, { }, { 'HTTPS' => 'on' }
			assert_equal current_path, new_user_session_path,
				"Didn't actually get new fake session path?"

			assert page.has_field?('user_session[username]'),
				"username field not found.  Probably didn't actually load new session page."
			assert page.has_field?('user_session[password]'),
				"password field not found.  Probably didn't actually load new session page."

			page.fill_in 'user_session[username]', :with => u.username
			page.fill_in 'user_session[password]', :with => 'V@1!dP@55w0rd'

			page.click_button 'Login'

			#	assert that there is a user logged in			
			assert_equal current_path, root_path,	#user_path(u),
