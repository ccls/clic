#	module ActiveRecordExtension; end
#	module ActiveRecordExtension::Errors
#		def self.included(base)
#			base.extend(ClassMethods)
#			base.send(:include,InstanceMethods)
#		end
#		module InstanceMethods
#			#
#			#	Can't believe this doesn't exist in rails.  It has proved very beneficial.
#			#
#			def on_attr_and_type(attribute,type)
#				attribute = attribute.to_s
#				return nil unless @errors.has_key?(attribute)
#				@errors[attribute].collect(&:type).include?(type)
#			end
#			alias_method :on_attr_and_type?, :on_attr_and_type
#		end
#		module ClassMethods
#			def delete(key)
#				@errors.delete(key.to_s)
#			end
#		end
#	end	#	ActiveRecordExtension::Errors
#	ActiveRecord::Errors.send(:include,
#		ActiveRecordExtension::Errors)


#	no longer valid in Rails 3
#	/Users/jakewendt/github_repo/ccls/clic.rails3/config/initializers/active_record_extension/errors.rb:24:in `<top (required)>': uninitialized constant ActiveRecord::Errors (NameError)
